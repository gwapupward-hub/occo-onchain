...
export function computeAgeScore(walletAgeDays: number): number {
  ...
  // Institutional credibility curve (non-linear):
  ...
  if (years >= 3) return 85 + ((years - 3) * 7.5); // 85→100 over 2 years
  if (years >= 2) return 70 + ((years - 2) * 15);  // 70→85 over 1 year
  if (years >= 1) return 40 + ((years - 1) * 30);  // 40→70 over 1 year
  return years * 40; // 0→40 in first year
}
...
export function computeActivityScore(
  ...
  // Log scale rewards consistent activity without over-rewarding spam
  const txCount = Math.max(0, safeInt(totalTransactionCount, 0));
  score += clamp(Math.log10(txCount + 1) * 5, 0, 20);
  
  // Component 2: Total volume (0-20 points)
  // Log scale rewards substantial borrowing volume
  const volume = Math.max(0, safeNum(totalVolumeUSD, 0));
  score += clamp(Math.log10(volume + 1) * 2, 0, 20);
  
  // Component 3: Position sizing consistency (0-10 points)
  // Rewards moderate, consistent position sizes (not wild swings)
  const avgSize = Math.max(0, safeNum(avgPositionSizeUSD, 0));
  const consistencyBonus = avgSize > 100 && avgSize < 100000 ? 10 : 5;
...
export function scoreWalletCreditV0(inputs: WalletCreditInputsV0): WalletCreditScoreResultV0 {
  const normalized: WalletCreditInputsV0 = {
    ...
    totalTransactionCount: Math.max(0, safeInt(inputs.totalTransactionCount ?? 0, 0)),
    totalVolumeUSD: Math.max(0, safeNum(inputs.totalVolumeUSD ?? 0, 0)),
    avgPositionSizeUSD: Math.max(0, safeNum(inputs.avgPositionSizeUSD ?? 0, 0)),
  };
  ...
  const activityScore = computeActivityScore(
    normalized.totalTransactionCount,
    normalized.totalVolumeUSD,
    normalized.avgPositionSizeUSD
  );
